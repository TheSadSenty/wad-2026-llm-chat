...
The application uses PostgreSQL and loads settings from `.env`.

- Default database URL: `postgresql+psycopg://postgres:changethis@127.0.0.1:5432/llm-chat`
- Local model path: `./model.gguf`
- Copy `.env.template` to `.env` and fill in the commented required secrets.
- Application settings use the `LLM_CHAT_` prefix.
- Example: `LLM_CHAT_DATABASE__POSTGRES__PORT=6432`
- Example: `LLM_CHAT_LLM__GGUF_PATH=/absolute/path/to/model.gguf`
...
Docker Compose reads variables from the same project-level `.env` file.
